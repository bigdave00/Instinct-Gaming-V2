	...
	elseif account == "society" then
		if billtype == "withdraw" then
			...
			elseif tonumber(society) >= amount then
				...
				else exports["qb-core-management"]:RemoveMoney(tostring(Player.PlayerData.job.name), amount) end
			end
		elseif billtype == "deposit" then
			...
			elseif bankB >= amount then
				...
				else exports["qb-core-management"]:AddMoney(tostring(Player.PlayerData.job.name), amount) end
				Player.Functions.RemoveMoney('bank', amount) Wait(1500)
	...
	elseif account == "gang" then
		if billtype == "withdraw" then
			...
			elseif tonumber(gsociety) >= amount then
				...
				else exports["qb-core-management"]:RemoveGangMoney(tostring(Player.PlayerData.gang.name), amount) end
			end
		elseif billtype == "deposit" then
			if bankB < amount then triggerNotify(nil, Loc[Config.Lan].error["nomoney_bank"], "error", src)
			elseif bankB >= amount then
				...
				else exports["qb-core-management"]:AddGangMoney(tostring(Player.PlayerData.gang.name), amount) end
				Player.Functions.RemoveMoney('bank', amount) Wait(1500)
...
QBCore.Functions.CreateCallback('jim-payments:ATM:Find', function(source, cb)
	...
	-- If qb-core-management, grab info directly from database
