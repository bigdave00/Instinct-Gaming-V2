...
Padlock.Close = function() {
    ...
    $.post('https://l2s-lockers/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://l2s-lockers/PadLockClose');
    if (data.error != null) {
        $.post('https://l2s-lockers/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://l2s-lockers/UseCombination", JSON.stringify({
        type: type,
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://l2s-lockers/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://l2s-lockers/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
...
function unlock() {
    gameOver = true;
    $.post('https://l2s-lockers/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
