...
for index, menu in ipairs({ 'SUBAMENU_GANG', 'SUBAMENU_CREATE', 'SUBAMENU_DELETE', 'SUBAMENU_MEMBER', 'SUBAMENU_MEMBER_MANAGE', 'SUBAMENU_MEMBERS', 'SUBAMENU_GARAGE', 'SUBAMENU_GARAGE_LIST', 'SUBAMENU_GARAGE_INSERT', 'SUBAMENU_GARAGE_REMOVE', 'SUBAMENU_CHECKPOINTS', 'SUBAMENU_CHECKPOINTS_MANAGE' }) do
...
function GangMenu(menu)
    ...
    if menu == 'MENU_HOME' then
        ...
        if zone and gang.access and not rivalry then
            if finishedRivalry then
                ...
                if not zone.gangName or zone.gangName == gang.name then
        ...
        if gang.superaccess and (Config.GangOptions.garage or Config.GangOptions.storage) then
        ...
        if gang.access then
            if gang.superaccess and #gang.members > 1 or #gang.members > 2 then
    ...
    if menu == 'SUBMENU_ACTIONS' then
        ...
        local alive = not IsEntityDead(targetPed)
    ...
    if menu == 'SUBMENU_MEMBERS' then
        ...
        for index, member in ipairs(gang.members) do
            if member.identifier ~= gang.identifier and member.identifier ~= identifier then
    ...
    if menu == 'SUBMENU_MANAGEMENT' then
        ...
        for index = 1, #Config.GangRanks - 1 do
            ranks[index] = Config.GangRanks[index].label
        end
        ...
        for index, member in ipairs(gang.members) do
            if member.identifier == identifier then
                if gang.superaccess then
                    ...
                    local pressed, access = Menu.CheckButton('ACCESS', Locale.MENU_MANAGEMENT_PERM, Config.GangRanks[rank].access, Config.GangRanks[rank].access)
    ...
    while Menu.CurrentMenu() do
        ...
        if menu == 'SUBAMENU_CREATE' then
            ...
            if Menu.Button('ACCEPT', Locale.MENU_BUTTON_CONFIRM, nil, true) then
                ...
                TriggerServerEvent('rcore_gangs:server:create_gang', tonumber(leader), color, tag, name)
            ...
            for index, member in ipairs(gang.members) do
                if member.identifier == identifier then
                    if gang.identifier == identifier then
                        Menu.Button('RANK', Locale.MENU_MANAGEMENT_RANK, Config.GangRanks[#Config.GangRanks].label, false)
