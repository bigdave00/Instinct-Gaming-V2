...
function StartHeist()
    QBCore.Functions.TriggerCallback('pacificheist:server:checkPoliceCount', function(status)
        if status then
            QBCore.Functions.TriggerCallback('pacificheist:server:checkTime', function(time)
                if time then
                    --ShowNotification(Strings['start_heist'])
                   -- ShowNotification(Strings['start_heist2'])
                    QBCore.Functions.Notify(Strings['start_heist'] , 'success', 5000)
                    QBCore.Functions.Notify(Strings['start_heist2'] , 'success', 15000)
...
Requested = {
    ...
    [2] = {name = QBCore.Shared.Items["bag"]["name"], image = QBCore.Shared.Items["bag"]["image"]},
    [3] = {name = QBCore.Shared.Items["cutter"]["name"], image = QBCore.Shared.Items["cutter"]["image"]},
    [4] = {name = QBCore.Shared.Items["c4_bomb"]["name"], image = QBCore.Shared.Items["c4_bomb"]["image"]},
    [5] = {name = QBCore.Shared.Items["thermite"]["name"], image = QBCore.Shared.Items["thermite"]["image"]},
    [6] = {name = QBCore.Shared.Items["laptop"]["name"], image = QBCore.Shared.Items["laptop"]["image"]},
    [7] = {name = QBCore.Shared.Items["trojan_usb"]["name"], image = QBCore.Shared.Items["trojan_usb"]["image"]},
}
    ...
    while true do
        ...
        if startDist <= 2.0 then
            ...
            if not requiredItemsShowed then
                ...
                TriggerEvent('inventory:client:requiredItems', Requested, true)
            end
            ...
            if requiredItemsShowed then
                ...
                TriggerEvent('inventory:client:requiredItems', Requested, false)
            end
        end
        for k, v in pairs(Config['PacificSetup']['freezeDoorList']) do
...
function AlertPolice()
    ...
    local DispatchData = {
        jobs = {["police"] = true},
        code = "15-01",
        callname = "Pacific Bank Robbery",
        coords = pos,
        info = {{
            icon = "fas fa-dollar-sign",
            label = "The big pacific bank being robbed, HURRY UP!"
        }},
        blip = { -- All the blip settings.
            label = "15-01", -- Blip label.
...
function LaptopHack(index)
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            QBCore.Functions.Notify(Strings['need_this'] .. itemLabel , 'error', 3500)
        end
        ...
        if hasItem then
            ...
            Drilling.Start(function(status)
                if status then
                    ...
                    exports['core_dispach']:addCall("20-12", "Pacific Bank Robbery Alarm!", {
                        {icon="fa-ruler", info="4.5 MILES"},
                        {icon="fa-crosshairs", info="MALE"}
                        }, {pedCo.x,pedCo.y,pedCo.z}, "police", 3000, 11, 5 )
                --    TriggerServerEvent('pacificheist:server:policeAlert', pedCo)
                else
...
function PaintingScene(sceneId)
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            if weapon ~= GetHashKey('WEAPON_SWITCHBLADE') then QBCore.Functions.Notify(Strings['need_switchblade'] , 'error', 3500) return end
...
function PlantingAnim(type, index, pos, rot)
    ...
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
          ...
          --  TriggerEvent("inventory:client:ItemBox", itemLabel, "remove")
            local ped = PlayerPedId()
            local pedCo, pedRotation = GetEntityCoords(ped), GetEntityRotation(ped)
...
function Outside(index)
    if robber then
        ...
        QBCore.Functions.Notify(Strings['deliver_to_buyer'], 'success', 3500)
        loadModel('baller')
