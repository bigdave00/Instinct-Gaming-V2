...
Requested = {
    [1] = {name = QBCore.Shared.Items["gasmask"]["name"], image = QBCore.Shared.Items["gasmask"]["image"]},
    ...
    [4] = {name = QBCore.Shared.Items["weapon_bzgas"]["name"], image = QBCore.Shared.Items["weapon_bzgas"]["image"]},
}
    ...
    QBCore.Functions.TriggerCallback('vangelicoheist:server:checkPoliceCount', function(status)
        if status then
            QBCore.Functions.TriggerCallback('vangelicoheist:server:checkTime', function(time)
                if time then
                    --ShowNotification(Strings['goto_vangelico'])
                    QBCore.Functions.Notify(Strings['goto_vangelico'] , 'success', 10000)
...
function AlertPolice()
    ...
    local DispatchData = {
        jobs = {["police"] = true},
        code = "10-07",
        callname = "Vangelico Robbery",
        coords = pos,
        info = {{
            icon = "fas fa-bahai",
            label = "Jewellery store being robbed, hurry up!"
        }},
        blip = { -- All the blip settings.
            label = "10-07", -- Blip label.
...
function VangelicoSetup()
    ...
    QBCore.Functions.Notify(Strings['good_shot'] , 'success', 5000)
    Wait(5000)
...
function Smash(index)
    ...
    if not weapon then QBCore.Functions.Notify(Strings['need_rifle'], 'error', 3500) return end
    QBCore.Functions.TriggerCallback('vangelicoheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            robber = true
            busy = true
            ...
            --ShowNotification(Strings['need_this'] .. itemLabel)
            QBCore.Functions.Notify(Strings['need_this'], 'error', 3500)
        end
    end, Config['VangelicoHeist']['requiredItems'][2])
end
