                    ...
                    if PoliceCount >= icfg.RequiredCops then
                        ...
                        elseif wasTalked then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                ...
                                QBCore.Functions.Notify("John: Find the model "  ..CarsToSteal.CarLabel.. " and bring the car to me!", "success")
                                addBlipParkingSpace()
                        ...
                        ShowFloatingHelpNotification("There are no active requests", vector3(icfg.CarThief.Boss.Pos.x, icfg.CarThief.Boss.Pos.y, icfg.CarThief.Boss.Pos.z + 0.9))
                    end
                elseif startedErrand then
                    if not hasComingGarage then
                        if not hasParked then
                            ...
                            DrawText3Ds(coords.x, coords.y, coords.z + 1.0, '[~r~H~s~] - Cancel the mission')
                            ...
                            if whatNext then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                   ...
                                   -- exports.pNotify:SendNotification({text = "<b>John</b></br>John told you to go to <b>" ..street2.. "</b> with <b style='color: #3bf5c3;'>" ..CarsToSteal.CarLabel.. "</b>", timeout = 3000})
                                    QBCore.Functions.Notify("John told you to go to the street "  ..street2.. " and park your car "  ..CarsToSteal.CarLabel.. ", in the barn!", "success")
                                elseif IsControlJustReleased(0, Keys['H']) then
                                    DoScreenFadeOut(FadeOut)
                                    Citizen.Wait(FadeOut)
                                    FreezeEntityPosition(ped, false)
                    ...
                    elseif hasComingGarage then
                        if WheelsCount > 0 and CarBodyCount > 0 and EngineCount > 0 then
                            ...
                            DrawText3Ds(coords.x, coords.y, coords.z + 1.0, '[~g~E~s~] - Receive payment')
                            if IsControlJustReleased(0, Keys["E"]) then
                                SetEntityCoords(ped, icfg.CarThief.Boss.Pos.x - 0.07, icfg.CarThief.Boss.Pos.y + 0.9, icfg.CarThief.Boss.Pos.z - 1.0)
                                SetEntityHeading(ped, 182.16)
                                RequestAnimDict("mp_common")
                    ...
                    if (#(coords - vector3(v.x, v.y, v.z)) < 1.8) then
                        if IsPedInAnyVehicle(ped, false) then
                            ...
                            if not v.hasParked then
                                if not hasParked then
                                    sleep = 5
                                    ShowFloatingHelpNotification("[~g~H~w~] - Park the stolen car", vector3(v.x, v.y, v.z + 1.2))
                                    if IsControlJustReleased(0, Keys['H']) then
                                        if cname == GetHashKey(CarsToSteal.CarModel) then
                                           ...
                                           -- exports.pNotify:SendNotification({text = "<b>John</b></br>Go to John to get nextly informations", timeout = 3000})
                                            QBCore.Functions.Notify("Go to john for the next set of instructions!", "success")
                                            
                                        else
                                            --exports.pNotify:SendNotification({text = "<b>John</b></br>This is not John's designated vehicle", timeout = 3000})
                                            QBCore.Functions.Notify("This is not the car requested by john...", "error")
                                        end
                                    end
                                end
                            elseif v.hasParked then
                                ShowFloatingHelpNotification("[~g~H~w~] - Take the stolen car out of the parking lot", vector3(v.x, v.y, v.z + 1.2))
                                if IsControlJustReleased(0, Keys['H']) then
...
function AlertPolice()
    ...
    local DispatchData = {
        jobs = {["police"] = true},
        code = "10-06",
        callname = "Vehicle Robbery",
        coords = pos,
        info = {{
            icon = "fas fa-car",
            label = "Reports about vehicle being stolen for a mission: Model - "..vehicleLabel..""
        }},
        blip = { -- All the blip settings.
            label = "10-06", -- Blip label.
                ...
                if searchingVehicle then
                    ...
                    if IsPedInAnyVehicle(ped, false) then
                        if cname == GetHashKey(CarsToSteal.CarModel) then
                            ...
                            QBCore.Functions.Notify("You found the brand car "  ..CarsToSteal.CarLabel.. ", steal it and go with it to john!", "success")
                                ...
                                if IsPedInAnyVehicle(ped, false) then
                                    ...
                                    if IsControlJustReleased(0, Keys['H']) then
                                        if cname == GetHashKey(CarsToSteal.CarModel) then
                                            ...
                                            QBCore.Functions.Notify("You have arrived at your destination, you can start disassembling the vehicle, you can start with the wheels, the body and finally the engine", "success")
                                            v.GarageParked = true
                                            startedWork = true
                                            ...
                                            --exports.pNotify:SendNotification({text = "<b>John</b></br>This is not John's designated vehicle", timeout = 3000})
                                            QBCore.Functions.Notify("This is not the vehicle, requested by john..", "error")
                                        end
                                    end
                                end
                            elseif (#(coords - vector3(v.x, v.y, v.z)) < 8.0) then
                            ...
                            if not IsPedInAnyVehicle(ped, false) then
                                        ...
                                        if not CarJackLSide then
                                            if (#(coords - vector3(LeftSide.x, LeftSide.y, LeftSide.z)) < 0.8) then
                                                if not hasWheel and not hasCarBody and not hasEngine and not hasEngineHoist then
                                                    ShowFloatingHelpNotification("[~g~E~w~] - Put the jack under the vehicle", vector3(LeftSide.x, LeftSide.y, LeftSide.z + 1.3))
                                                    if IsControlJustReleased(0, Keys["E"]) then
                                                        exports.rprogress:Custom({
                                                            Duration = 3500,
                                                            Label = "Putting the jack..",
                                                            DisableControls = {
                                                                Mouse = false,
                                                                Player = true,
                                                                Vehicle = true
                                            ...
                                            if (#(coords - vector3(LeftSide.x, LeftSide.y, LeftSide.z)) < 0.8) then
                                                ...
                                                if not hasWheel and not hasCarBody and not hasEngine and not hasEngineHoist then
                                                    if RotCarJack == 0.0 then
                                                        DrawText3Ds(LeftSide.x, LeftSide.y, LeftSide.z + 0.4, '[~g~E~s~] - Pick up the vehicle | [~r~H~s~] - Take out the jack')
                                                        if IsControlJustReleased(0, Keys["E"]) then
                                                    ...
                                                    elseif RotCarJack > 0.0 and RotCarJack < 5.0 then
                                                        DrawText3Ds(LeftSide.x, LeftSide.y, LeftSide.z + 0.4, '[~g~E~s~] - Pick up the vehicle | [~g~H~s~] - Get off the vehicle')
                                                        if IsControlJustReleased(0, Keys["E"]) then
                                                            RotCarJack = RotCarJack + 1.0
                                                            SetEntityRotation(vehicle, LastRotation.x, RotCarJack, LastRotation.z)
                                                        elseif IsControlJustReleased(0, Keys['H']) then
                                                    ...
                                                    elseif RotCarJack == 5.0 then
                                                        DrawText3Ds(LeftSide.x, LeftSide.y, LeftSide.z + 0.4, '[~g~H~s~] - Get off the vehicle')
                                                        if IsControlJustReleased(0, Keys['H']) then
                                                            RotCarJack = RotCarJack - 1.0
                                                            SetEntityRotation(vehicle, LastRotation.x, RotCarJack, LastRotation.z)
                                                        end
                                    ...
                                    if not CarJackLSide then -- Right Side
                                        if not CarJackRSide then
                                            if (#(coords - vector3(RightSide.x, RightSide.y, RightSide.z)) < 0.8) then
                                                if not hasWheel and not hasCarBody and not hasEngine and not hasEngineHoist then
                                                    ShowFloatingHelpNotification("[~g~E~w~] - Put the jack under the vehicle", vector3(RightSide.x, RightSide.y, RightSide.z + 1.3))
                                                    if IsControlJustReleased(0, Keys["E"]) then
                                                        exports.rprogress:Custom({
                                                            Duration = 3500,
                                                            Label = "Putting the jack..",
                                        ...
                                        elseif CarJackRSide then
                                            if (#(coords - vector3(RightSide.x, RightSide.y, RightSide.z)) < 0.8) then
                                                ...
                                                if not hasWheel and not hasCarBody and not hasEngine and not hasEngineHoist then
                                                    if RotCarJack == 0.0 then
                                                        DrawText3Ds(RightSide.x, RightSide.y, RightSide.z + 0.4, '[~g~E~s~] - Pick up the vehicle | ~r~[H]~s~ - Take out the jack')
                                                        if IsControlJustReleased(0, Keys["E"]) then
                                                            RotCarJack = RotCarJack - 1.0
                                                            SetEntityRotation(vehicle, LastRotation.x, RotCarJack, LastRotation.z)
                                                        elseif IsControlJustReleased(0, Keys["H"]) then
                                                    ...
                                                    elseif RotCarJack < 0.0 and RotCarJack > -5.0 then
                                                        DrawText3Ds(RightSide.x, RightSide.y, RightSide.z + 0.4, '[~g~E~s~] - Pick up the vehicle | [~g~H~s~] - Get off the vehicle')
                                                        if IsControlJustReleased(0, Keys["E"]) then
                                                            RotCarJack = RotCarJack - 1.0
                                                            SetEntityRotation(vehicle, LastRotation.x, RotCarJack, LastRotation.z)
                                                        elseif IsControlJustReleased(0, Keys['H']) then
                                                    ...
                                                    elseif RotCarJack == -5.0 then
                                                        DrawText3Ds(RightSide.x, RightSide.y, RightSide.z + 0.4, '[~g~H~s~] - Get off the vehicle')
                                                        if IsControlJustReleased(0, Keys['H']) then
                                                            RotCarJack = RotCarJack + 1.0
                                                            SetEntityRotation(vehicle, LastRotation.x, RotCarJack, LastRotation.z)
                                                        end
                                    ...
                                    if (#(coords - vector3(FrontLeftWheel.x, FrontLeftWheel.y, FrontLeftWheel.z)) < 0.8) then -- Front Left Wheel
                                        if not FLW then
                                            if not hasWheel then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Take out the wheel", vector3(FrontLeftWheel.x, FrontLeftWheel.y, FrontLeftWheel.z + 0.4))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Taking out wheel..",
                                                        DisableControls = {
                                                            Mouse = false,
                                                            Player = true,
                                                            Vehicle = true
                                            ...
                                            elseif hasWheel then
                                                ShowFloatingHelpNotification("Wheels to unpack", vector3(FrontLeftWheel.x, FrontLeftWheel.y, FrontLeftWheel.z + 0.4))
                                            end
                                        elseif FLW then
                                            ShowFloatingHelpNotification("~r~Unpacked wheel~s~", vector3(FrontLeftWheel.x, FrontLeftWheel.y, FrontLeftWheel.z + 0.4))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    elseif (#(coords - vector3(RearLeftWheel.x, RearLeftWheel.y, RearLeftWheel.z)) < 0.8) then -- Rear Left Wheel
                                        if not RLW then
                                            if not hasWheel then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Take out the wheel", vector3(RearLeftWheel.x, RearLeftWheel.y, RearLeftWheel.z + 0.4))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Taking out wheel...",
                                            ...
                                            elseif hasWheel then
                                                ShowFloatingHelpNotification("Wheels to unpack", vector3(RearLeftWheel.x, RearLeftWheel.y, RearLeftWheel.z + 0.4))
                                            end
                                        elseif RLW then
                                            ShowFloatingHelpNotification("~r~Unpacked wheel~s~", vector3(RearLeftWheel.x, RearLeftWheel.y, RearLeftWheel.z + 0.4))
                                        end
                                    end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                elseif RotCarJack == -5.0 then
                                    ...
                                    if (#(coords - vector3(FrontRightWheel.x, FrontRightWheel.y, FrontRightWheel.z)) < 0.8) then -- Front Right Wheel
                                        if not FRW then
                                            if not hasWheel then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Take out the wheel", vector3(FrontRightWheel.x, FrontRightWheel.y, FrontRightWheel.z + 0.4))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Taking out wheel...",
                                            ...
                                            elseif hasWheel then
                                                ShowFloatingHelpNotification("Wheels to unpack", vector3(FrontRightWheel.x, FrontRightWheel.y, FrontRightWheel.z + 0.4))
                                            end
                                        elseif FRW then
                                            ShowFloatingHelpNotification("~r~Unpacked wheel~s~", vector3(FrontRightWheel.x, FrontRightWheel.y, FrontRightWheel.z + 0.4))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    elseif (#(coords - vector3(RearRightWheel.x, RearRightWheel.y, RearRightWheel.z)) < 0.8) then -- Rear Right Wheel
                                        if not RRW then
                                            if not hasWheel then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Take out the wheel", vector3(RearRightWheel.x, RearRightWheel.y, RearRightWheel.z + 0.4))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Taking out wheel...",
                                            ...
                                            elseif hasWheel then
                                                ShowFloatingHelpNotification("Wheels to unpack", vector3(RearRightWheel.x, RearRightWheel.y, RearRightWheel.z + 0.4))
                                            end
                                        elseif RRW then
                                            ShowFloatingHelpNotification("~r~Unpacked wheel~s~", vector3(RearRightWheel.x, RearRightWheel.y, RearRightWheel.z + 0.4))
                                        end
                                    end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    ...
                                    if (#(coords - vector3(FrontLeftDoor.x, FrontLeftDoor.y, FrontLeftDoor.z)) < 0.8) then -- Front Left Door
                                        if not FLD then
                                            if not hasCarBody then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Remove the door", vector3(FrontLeftDoor.x, FrontLeftDoor.y, FrontLeftDoor.z + 0.8))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Remove the door screws...",
                                                        DisableControls = {
                                                            Mouse = false,
                                                            Player = true,
                                                            Vehicle = true
                                            ...
                                            elseif hasCarBody then
                                                ShowFloatingHelpNotification("Disassemble door", vector3(FrontLeftDoor.x, FrontLeftDoor.y, FrontLeftDoor.z + 0.8))
                                            end
                                        elseif FLD then
                                            ShowFloatingHelpNotification("~r~Disassembled door~s~", vector3(FrontLeftDoor.x, FrontLeftDoor.y, FrontLeftDoor.z + 0.8))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    elseif (#(coords - vector3(RearLeftDoor.x, RearLeftDoor.y, RearLeftDoor.z)) < 0.8) then -- Rear Left Door
                                        if not RLD then
                                            if not hasCarBody then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Remove the door", vector3(RearLeftDoor.x, RearLeftDoor.y, RearLeftDoor.z + 0.8))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Remove the door screws...",
                                            ...
                                            elseif hasCarBody then
                                                ShowFloatingHelpNotification("Disassemble door", vector3(RearLeftDoor.x, RearLeftDoor.y, RearLeftDoor.z + 0.8))
                                            end
                                        elseif RLD then
                                            ShowFloatingHelpNotification("~r~Disassembled door~s~", vector3(RearLeftDoor.x, RearLeftDoor.y, RearLeftDoor.z + 0.8))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    elseif (#(coords - vector3(FrontRightDoor.x, FrontRightDoor.y, FrontRightDoor.z)) < 0.8) then -- Front Right Door
                                        if not FRD then
                                            if not hasCarBody then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Remove the door", vector3(FrontRightDoor.x, FrontRightDoor.y, FrontRightDoor.z + 0.8))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Remove the door screws...",
                                            ...
                                            elseif hasCarBody then
                                                ShowFloatingHelpNotification("Disassemble door", vector3(FrontRightDoor.x, FrontRightDoor.y, FrontRightDoor.z + 0.8))
                                            end
                                        elseif FRD then
                                            ShowFloatingHelpNotification("~r~Disassembled door~s~", vector3(FrontRightDoor.x, FrontRightDoor.y, FrontRightDoor.z + 0.8))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    elseif (#(coords - vector3(RearRightDoor.x, RearRightDoor.y, RearRightDoor.z)) < 0.8) then -- Rear Right Door
                                        if not RRD then
                                            if not hasCarBody then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Remove the door", vector3(RearRightDoor.x, RearRightDoor.y, RearRightDoor.z + 0.8))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Remove the door screws...",
                                            ...
                                            elseif hasCarBody then
                                                ShowFloatingHelpNotification("Disassemble door", vector3(RearRightDoor.x, RearRightDoor.y, RearRightDoor.z + 0.8))
                                            end
                                        elseif RRD then
                                            ShowFloatingHelpNotification("~r~Disassembled door~s~", vector3(RearRightDoor.x, RearRightDoor.y, RearRightDoor.z + 0.8))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    elseif (#(coords - vector3(FrontDoor.x, FrontDoor.y, FrontDoor.z)) < 0.8) then -- Front Door
                                        if not FD then
                                            if not hasCarBody then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Remove the hood", vector3(FrontDoor.x, FrontDoor.y, FrontDoor.z + 0.6))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    SetVehicleDoorOpen(vehicle, 4, false, false)
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Removing the bonnet bolts...",
                                                        DisableControls = {
                                                            Mouse = false,
                                                            Player = true,
                                                            Vehicle = true
                                            ...
                                            elseif hasCarBody then
                                                ShowFloatingHelpNotification("Disassembly hood", vector3(FrontDoor.x, FrontDoor.y, FrontDoor.z + 0.6))
                                            end
                                        elseif FD then
                                            ShowFloatingHelpNotification("~r~Hood removed~s~", vector3(FrontDoor.x, FrontDoor.y, FrontDoor.z + 0.6))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                    elseif (#(coords - vector3(RearDoor.x, RearDoor.y, RearDoor.z)) < 0.8) then -- Rear Door
                                        if not RD then
                                            if not hasCarBody then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Remove the trunk", vector3(RearDoor.x, RearDoor.y, RearDoor.z + 0.6))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    SetVehicleDoorOpen(vehicle, 5, false, false)
                                                    exports.rprogress:Custom({
                                                        Duration = 7000,
                                                        Label = "Taking out trunk screws...",
                                                        DisableControls = {
                                                            Mouse = false,
                                                            Player = true,
                                                            Vehicle = true
                                            ...
                                            elseif hasCarBody then
                                                ShowFloatingHelpNotification("Disassembly trunk", vector3(RearDoor.x, RearDoor.y, RearDoor.z + 0.6))
                                            end
                                        elseif RD then
                                            ShowFloatingHelpNotification("~r~Trunk disassembled~s~", vector3(RearDoor.x, RearDoor.y, RearDoor.z + 0.6))
                                        end
                                    -------------------------------------------------------------------------------------------------------------------------------------------------
                                            ...
                                            if not Engine then
                                                if not hasEngine then
                                                    if hasEngineHoist then
                                                        ShowFloatingHelpNotification("[~g~E~w~] - Remove the engine", vector3(FrontSide.x - 0.75, FrontSide.y + 1.5, FrontSide.z + 0.6))
                                                        if IsControlJustReleased(0, Keys["E"]) then
                                                            exports.rprogress:Custom({
                                                                Duration = 10000,
                                                                Label = "Removing the engine bolts...",
                                                                DisableControls = {
                                                                    Mouse = false,
                                                                    Player = true,
                                                                    Vehicle = true
                                                ...
                                                elseif hasEngine then
                                                    ShowFloatingHelpNotification("Disassembly engine", vector3(FrontSide.x, FrontSide.y, FrontSide.z + 0.6))
                                                end
                                            elseif Engine then
                                ...
                                if CarBodyCount > 0 then
                                    if (#(coords - vector3(icfg.CarThief.CarBodyPlace.Pos.x, icfg.CarThief.CarBodyPlace.Pos.y, icfg.CarThief.CarBodyPlace.Pos.z)) < 1.2) then -- Place to put down Car Body
                                        ...
                                        if hasCarBody then
                                            ShowFloatingHelpNotification("[~g~E~w~] - Store components", vector3(icfg.CarThief.CarBodyPlace.Pos.x, icfg.CarThief.CarBodyPlace.Pos.y, icfg.CarThief.CarBodyPlace.Pos.z + 0.9))
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                exports.rprogress:Custom({
                                                    Duration = 2500,
                                                    Label = "The body components are stored...",
                                                    DisableControls = {
                                                        Mouse = false,
                                                        Player = true,
                                                        Vehicle = true
                                                ...
                                                if CarBodyCount == 0 and WheelsCount > 0 then
                                                    ...
                                                    QBCore.Functions.Notify("Awesome, you disassembled the whole body, you still need the wheels. You can find a jack in the garage for this!", "success")
                                                elseif CarBodyCount == 0 and WheelsCount == 0 then
                                                    ...
                                                    QBCore.Functions.Notify("The vehicle is almost dismantled, you only have to work on the engine!", "success")
                                                end
                                            end
                                        elseif not hasCarBody then
                                            ShowFloatingHelpNotification("You still have to disassemble " ..CarBodyCount.. " components, ", vector3(icfg.CarThief.CarBodyPlace.Pos.x, icfg.CarThief.CarBodyPlace.Pos.y, icfg.CarThief.CarBodyPlace.Pos.z + 0.9))
                                        end
                                ...
                                if WheelsCount > 0 then
                                    if (#(coords - vector3(icfg.CarThief.WheelsPlace.Pos.x, icfg.CarThief.WheelsPlace.Pos.y, icfg.CarThief.WheelsPlace.Pos.z)) < 1.2) then -- Place to put down Wheels
                                        ...
                                        if hasWheel then
                                            ShowFloatingHelpNotification("[~g~E~w~] - Put the wheel down", vector3(icfg.CarThief.WheelsPlace.Pos.x, icfg.CarThief.WheelsPlace.Pos.y, icfg.CarThief.WheelsPlace.Pos.z + 0.9))
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                exports.rprogress:Custom({
                                                    Duration = 2500,
                                                    Label = "Putting wheel down...",
                                                    DisableControls = {
                                                        Mouse = false,
                                                        Player = true,
                                                        Vehicle = true
                                                ...
                                                if WheelsCount == 0 and CarBodyCount > 0 then
                                                    ...
                                                    QBCore.Functions.Notify("The wheels have been dismantled, start the body. Take the jack back to the garage.", "success")
                                                elseif CarBodyCount == 0 and WheelsCount == 0 then
                                                   -- exports.pNotify:SendNotification({text = "<b>John</b></br>Car is almost ready, you only have Engine to dismantle. Return Car Jack in its place and you're ready to start!", timeout = 5000})
                                                    QBCore.Functions.Notify("The vehicle is almost disassembled, you have more than the engine. Take the jack in the garage and start the engine!", "success")
                                                end
                                            end
                                        elseif not hasWheel then
                                            ShowFloatingHelpNotification("You still have " ..WheelsCount.. " disassembly wheels", vector3(icfg.CarThief.WheelsPlace.Pos.x, icfg.CarThief.WheelsPlace.Pos.y, icfg.CarThief.WheelsPlace.Pos.z + 0.9))
                                        end
                                ...
                                if EngineCount > 0 then
                                    if WheelsCount == 0 and CarBodyCount == 0 then
                                        if (#(coords - vector3(icfg.CarThief.EnginePlace.Pos.x, icfg.CarThief.EnginePlace.Pos.y, icfg.CarThief.EnginePlace.Pos.z)) < 1.2) then -- Place to put down Engine
                                            ...
                                            if hasEngine then
                                                ShowFloatingHelpNotification("[~g~E~w~] - Put the engine down", vector3(icfg.CarThief.EnginePlace.Pos.x, icfg.CarThief.EnginePlace.Pos.y, icfg.CarThief.EnginePlace.Pos.z + 0.9))
                                                if IsControlJustReleased(0, Keys["E"]) then
                                                    exports.rprogress:Custom({
                                                        Duration = 5500,
                                                        Label = "Putting engine down...",
                                                        DisableControls = {
                                                            Mouse = false,
                                                            Player = true,
                                                            Vehicle = true
                                                    ...
                                                    QBCore.Functions.Notify("Great, take the engine pulley back to the garage!", "success")
                                                end
                                            elseif not hasEngine then
                                                ShowFloatingHelpNotification("You did not disassemble the engine", vector3(icfg.CarThief.EnginePlace.Pos.x, icfg.CarThief.EnginePlace.Pos.y, icfg.CarThief.EnginePlace.Pos.z + 0.9))
                                            end
                                ...
                                if (#(coords - vector3(icfg.CarThief.Hoist.Pos.x, icfg.CarThief.Hoist.Pos.y, icfg.CarThief.Hoist.Pos.z)) < 1.2) then -- Hoist
                                    ...
                                    if not hasCarBody and not hasEngine and not hasWheel and not hasCarJack then
                                        if not hasEngineHoist then
                                            ShowFloatingHelpNotification("[~g~E~w~] - Use engine pulleys", vector3(icfg.CarThief.Hoist.Pos.x, icfg.CarThief.Hoist.Pos.y, icfg.CarThief.Hoist.Pos.z + 0.9))
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                exports.rprogress:Custom({
                                                    Duration = 3500,
                                                    Label = "Removing the engine pulley from the garage..",
                                                    DisableControls = {
                                                        Mouse = false,
                                                        Player = true,
                                                        Vehicle = true
                                        ...
                                        elseif hasEngineHoist then
                                            ShowFloatingHelpNotification("[~g~E~w~] - Returns engine pulleys", vector3(icfg.CarThief.Hoist.Pos.x, icfg.CarThief.Hoist.Pos.y, icfg.CarThief.Hoist.Pos.z + 0.9))
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                exports.rprogress:Custom({
                                                    Duration = 3500,
                                                    Label = "Taking the pulley back to the garage...",
                                                    DisableControls = {
                                                        Mouse = false,
                                                        Player = true,
                                                        Vehicle = true
                                                ...
                                                if EngineCount == 0 then
                                                    ...
                                                    QBCore.Functions.Notify("Great job, go to john now to collect your payment!", "success")
                                                    QBCore.Functions.DeleteVehicle(vehicle)
                                ...
                                if (#(coords - vector3(icfg.CarThief.CarJack.Pos.x, icfg.CarThief.CarJack.Pos.y, icfg.CarThief.CarJack.Pos.z)) < 1.2) then -- Car Jack
                                    ...
                                    if not hasCarBody and not hasEngine and not hasWheel and not hasEngineHoist then
                                        if not hasCarJack then
                                            ShowFloatingHelpNotification("[~g~E~w~] - Use a jack", vector3(icfg.CarThief.CarJack.Pos.x, icfg.CarThief.CarJack.Pos.y, icfg.CarThief.CarJack.Pos.z + 0.9))
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                exports.rprogress:Custom({
                                                    Duration = 2000,
                                                    Label = "Using the jack for the car...",
                                                    DisableControls = {
                                                        Mouse = false,
                                                        Player = true,
                                                        Vehicle = true
                                        ...
                                        elseif hasCarJack then
                                            ShowFloatingHelpNotification("[~g~E~w~] - Return the jack", vector3(icfg.CarThief.CarJack.Pos.x, icfg.CarThief.CarJack.Pos.y, icfg.CarThief.CarJack.Pos.z + 0.9))
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                exports.rprogress:Custom({
                                                    Duration = 2000,
                                                    Label = "Take the jack back to the garage...",
                                                    DisableControls = {
                                                        Mouse = false,
                                                        Player = true,
                                                        Vehicle = true
