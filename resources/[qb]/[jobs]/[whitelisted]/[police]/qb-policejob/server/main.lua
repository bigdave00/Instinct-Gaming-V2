                ...
                if amount and amount > 0 then
                    ...
                    elseif billed.Functions.RemoveMoney('cash', amount, "paid-fine") then
                        ...
                        MySQL.Async.insert('INSERT INTO phone_invoices (citizenid, amount, society, sender, sendercitizenid) VALUES (?, ?, ?, ?, ?)',{billed.PlayerData.citizenid, amount, biller.PlayerData.job.name, biller.PlayerData.charinfo.firstname, biller.PlayerData.citizenid}, function(id)
                            if id then
                                TriggerClientEvent('qs-smartphone:client:AcceptorDenyInvoice', billed.PlayerData.source, id, biller.PlayerData.charinfo.firstname, biller.PlayerData.job.name, biller.PlayerData.citizenid, amount, GetInvokingResource())
                            end
                        end)
...
RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("qs-smartphone:client:addPoliceAlert", requestId, alertData)
end)

QBCore.Commands.Add('999police', Lang:t("commands.police_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
	local src = source
    local message
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'police' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qs-smartphone:client:addPoliceAlert", v.PlayerData.source, alertData)
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["inlaststand"] or EscortPlayer.PlayerData.metadata["isdead"]) then
...
RegisterNetEvent('police:server:KidnapPlayer', function(playerId)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["inlaststand"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
...
RegisterNetEvent('police:server:SetPlayerOutVehicle', function(playerId)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
