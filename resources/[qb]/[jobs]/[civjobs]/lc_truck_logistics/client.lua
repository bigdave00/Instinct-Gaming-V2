...
AddEventHandler('truck_logistics:spawnTruck', function(truck_data)
	...
	truck_data.properties.plate = truck_data.properties.plate or Utils.Vehicles.generateTempVehiclePlate(Utils.translate('truck_plate'))
...
AddEventHandler('truck_logistics:startContract', function(key,contract_data,location)
	...
	if contract_data.contract_type == 0 then
		...
		local blip_data = { name = Utils.translate('rented_truck_blip'), sprite = 477, color = 26 }
		local properties = { plate = Utils.Vehicles.generateTempVehiclePlate(Utils.translate('truck_plate')) }
		truck,truck_blip = Utils.Vehicles.spawnVehicle(contract_data.truck,x,y,z,h,blip_data,properties)
		rent_truck = true
	end
	...
	local blip_data = { name = Utils.translate('cargo_blip'), sprite = 479, color = 26 }
	local properties = { plate = Utils.Vehicles.generateTempVehiclePlate(Utils.translate('truck_plate')) }
	trailer,trailer_blip = Utils.Vehicles.spawnVehicle(contract_data.trailer,x,y,z,h,blip_data,properties)
