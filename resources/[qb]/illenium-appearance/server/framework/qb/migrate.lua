...
local function MigrateQBClothing(source)
    ...
    for i=1, #allPlayerSkins, 1 do
        if not tonumber(allPlayerSkins[i].model) then
            ...
            TriggerClientEvent("illenium-appearance:client:migration:load-illenium-appearance-skin", source, allPlayerSkins[i])
...
RegisterNetEvent("illenium-appearance:server:migrate-illenium-appearance-skin", function(citizenid, appearance)
    local src = source
