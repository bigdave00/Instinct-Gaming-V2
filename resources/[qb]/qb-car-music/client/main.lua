...
function show(nomecenas)
    ...
    elseif nuiaberto then
		...
		QBCore.Functions.Notify("You are not in the vehicle!")
	end
end
			...
			if myjob.name == v.job then
				...
				if distance <= 10 then
					...
					if distance <= 1.5 then
						dormir = 5
						QBCore.Functions.DrawText3D(v.changemusicblip.x, v.changemusicblip.y, v.changemusicblip.z, "~r~E~w~ - Şarkı Oynat")
